...
--- 
Few-shot relation extraction achieves great progress by incorporating meta-learning with knowledge. Most of the recent works focus on integrating different types of knowledge, which ignored that there would be a huge knowledge gap in the application, especially facing domain adaptation. In this paper, we explore the effective way to integrate different forms of knowledge for domain adaptation few-shot relation extraction, and we face up to the problem of knowledge gap between  Cites: PTR: Prompt tuning with rules for text classification