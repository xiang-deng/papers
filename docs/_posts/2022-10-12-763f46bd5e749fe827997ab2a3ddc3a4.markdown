--- 
layout: post 
title: "Emotive facets of place meet urban analytics" 
date: 2022-10-12 20:42:55 -0400 
categories: jekyll update 
author: "A Acedo, A GonzlezPrez, C Granell, S Casteleyn - Transactions in GIS, 2022" 
--- 
The lack of a wellestablished and unified place theory across disciplines is decelerating its formalization, evolution, and especially its pragmatic implications and applicability. In this article, we identify research gaps in the emotive facets of place scholarship. We found that it:(1) rarely joins physical, social, and individual variables in the same model;(2) omits the immediately perceived and sensory dimensions;(3) disregards the analysis of how individualplace emotive  Cites: Sentihood: Targeted aspect based sentiment analysis dataset for