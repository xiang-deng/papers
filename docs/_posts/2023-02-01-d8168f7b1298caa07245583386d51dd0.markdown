--- 
layout: post 
title: "Understanding the Cognitive Influences of Interpretability Features on How Users Scrutinize Machine-Predicted Categories" 
date: 2023-02-01 14:37:22 -0400 
categories: jekyll update 
author: "J Qu, J Arguello, Y Wang - 2023" 
--- 
The goal of interpretable machine learning (ML) is to design tools and visualizations to help users scrutinize a system s predictions. Prior studies have mostly employed quantitative methods to investigate the effects of specific tools/visualizations on outcomes related to objective performancea human s ability to correctly agree or disagree with the systemand subjective perceptions of the system. Few studies have employed qualitative methods to investigate how and why specific  Cites: Does the whole exceed its parts? the effect of ai explanations on