--- 
layout: post 
title: "Multirelational Collaborative Filtering for Global Graph Neural Networks to Mine Evolutional Social Relations" 
date: 2022-12-27 00:23:06 -0400 
categories: jekyll update 
author: "X Deng, P Jiang, X Chen - IEEE Transactions on Computational Social Systems, 2022" 
--- 
Due to the unstable and complex social network environment, the sole useritem interaction data become insufficient for generating precise recommendations. However, too much emphasis on useritem interactions prevents the discovery of internal connections among them, such as trustworthy user relations. In this work, we have integrated the collaborative and the sequential relations into an end-to-end graph neural network (GNN) simultaneously and proposed a novel framework Cites: Recommender systems with social regularization